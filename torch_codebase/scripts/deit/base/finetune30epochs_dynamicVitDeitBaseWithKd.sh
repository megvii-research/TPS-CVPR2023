...
dynamic_prune_lr_sch_opt_loss_settings="--batch-size 512  --lr 1.25e-4 --unscale-lr  --epochs 30 --weight-decay 0.05 --sched cosine   --opt adamw  --warmup-lr 1e-6 --warmup-epochs 5 --backbone_freeze_epochs -1 --backbone_lr_scale 1e-2"
# 不确定dynamicViT用了没
...
# rerun 下训蹦的实验
keep_ratio_list=(0.5)
prune_loc_list=([3,5,7,9])



arch_list_small=(dynamic_deit_base_patch16_224)
    ...
    do  
        ...
        do 
            ...
            do     
                ...
                while : ; do
                    CUDA_HOME=/data/cuda/cuda-10.2/cuda/ rlaunch --charged-group=${charged_group} --preemptible=${preemptible}  --positive-tags 2080ti --cpu=48 --gpu=8 --memory=128000 \
                    -- python3 -m torch.distributed.launch --nproc_per_node=8 --use_env train.py \
                    -c  -dist-eval --pin-mem  --dataset ImageNet \
                    --grad_checkpoint --grad_checkpoint_layers 0 1 2 3 4 5 6 \
