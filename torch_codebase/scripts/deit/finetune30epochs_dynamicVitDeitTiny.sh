...
arch_list_tiny=(dynamic_deit_tiny_patch16_224_token_merge_softmax_reweight)
# arch_list_tiny=(dynamic_deit_tiny_patch16_224_token_merge_use_dots_softmax_reweight)
for prune_loc in ${prune_loc_list[@]};
do 
    for keep_ratio in ${keep_ratio_list[@]};
    do  
        for arch in ${arch_list_tiny[@]};
        do  
