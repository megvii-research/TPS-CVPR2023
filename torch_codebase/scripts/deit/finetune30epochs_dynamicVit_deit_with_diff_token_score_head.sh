...
# token_score_head_list=(LocalPredictor2Layer LocalPredictor3Layer TokenResErrScoreHead2Layer TokenDiffScoreHead2Layer)
token_score_head_list=(TokenResErrScoreHead2Layer TokenDiffScoreHead2Layer)
...
do 
    ...
    do  
        ...
        do 
            for token_score_head in ${token_score_head_list[@]};
            do
                ...
                do     
                    ...
                    prune_loc_str="${prune_loc_str//,/}"
                    task_name="${USER}-${prefix}-${arch}-${keep_ratio}-${prune_loc_str}-${token_score_head}-rerun${rerun_i}"
                    echo ${task_name} ;
                    ...
                    while : ; do
                        ...
                        --grad_checkpoint --grad_checkpoint_layers 0  \
                        --arch $arch ${deit1_dynamicViT_args} \
                        --arch_kwargs "dict( prune_loc=${prune_loc},keep_ratio=${keep_ratio},token_score_head_cls='${token_score_head}' )" \
...
# arch_list=(dynamic_deit_small_patch16_224_CosSimNearestPrunedTokenProjAggregate_mean)
# token_score_head_list=(TokenResErrScoreHead2Layer TokenDiffScoreHead2Layer)
# for prune_loc in ${prune_loc_list[@]};
# do 
#     for keep_ratio in ${keep_ratio_list[@]};
...
#                     while : ; do
#                         rlaunch --charged-group=${charged_group} --preemptible=${preemptible}  --positive-tags 2080ti --cpu=48 --gpu=8 --memory=80000 \
